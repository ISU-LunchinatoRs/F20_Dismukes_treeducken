...
Hosts and their parasites or mutualists (we're going to call these symbionts) that are ecologically dependent are thought to evolve together through time.
This is thought to cause the phylogenies (i.e. evolutionary trees) of the host and the symbiont to be identical.
This is not often seen in empirical systems so researchers have devised two main methods of examining this data: global-fit methods and event-based methods. 
Global-fit methods infer the shared phylogenetic signal between the two trees, asking the question of whether the two trees and the ecological associations at the tips are more similar to each other than we would expect due to chance.
Event-based methods use events such as cospeciation (when a host and symbiont speciate simultaneously) or host-switching to explain the differences between the host and symbiont tree.
Each event is given a cost, and the events are used to map the symbiont tree onto the host tree with the intent of finding the mapping with the lowest cost.

Inherent in these event-based methods is the idea that the two trees are evolving in tandem and that these events occur at different rates.
In other words, there is an implicit model that we assume generates our data. 
However, few of these event-based methods use statistical models as the basis for the event costs, and there are only a handful of statistical models for generating this data (Keller et al. 2012, Baudet et al. 2015).
Neither of these models allows for extinction in the host and symbiont trees.
We provide an additional generative model that accounts for extinction, and also it is R which is a bit nicer than a command line interface program.
